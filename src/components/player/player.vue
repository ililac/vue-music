...
export default {
    ...
    methods: {
        ...
        format(interval) { // 格式化时间戳
            interval = interval | 0 // 正数向下取整 === Math.floor()
            const mimute = interval / 60 | 0
            const second = this._pad(interval % 60)
            return `${mimute}:${second}`
        },
        _pad(num, n = 2) { // 补0  n为需要补的位数
        ...
        onProgressBarChange(percent) { // 拖动进度条改变播放进度
